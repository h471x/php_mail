...
$query = "SELECT email_destination_fullname, email_destination, objet, contenu, send_time, send_date FROM message WHERE email_user = ? ORDER BY id_message DESC";
...
$emailRows = array_map(function($row) {
    return [
        "fullname" => $row["email_destination_fullname"],
...
function generateEmailRows($rows) {
    foreach ($rows as $row) {
        echo '
        <div class="sentRow">
            <h3 class="sentRow__title"<span class="receiver_info"></span>&nbsp;' . $row["fullname"] . '</h3>
            ...
            <div class="sent_sender" style="display: none;">' . $row['fullname'] .' &lt;' . $row['mail'] . '&gt;</div>
